  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hallo, ich bin dein **Bubble Guide** ✨\n\nIch begleite dich sanft durch Koulners Bubbles und kenne all unsere Artikel zu Natur, Philosophie, Gesundheit, Kosmetik, Ernährung, Frequenzen und Funktionellem Training.\n\nWas möchtest du heute für dein Wohlbefinden erfahren?",
    },
  ]);
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className={`fixed inset-0 w-full h-full md:inset-auto md:bottom-28 md:right-6 z-50 bg-surface/95 md:bg-surface/95 backdrop-blur-xl border-0 md:border border-sand/50 rounded-none md:rounded-3xl shadow-2xl flex flex-col overflow-hidden text-text-main transition-all duration-300 ${isExpanded
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 text-sm">
                {messages.map((msg, idx) => (
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-xs ${msg.role === "user"
